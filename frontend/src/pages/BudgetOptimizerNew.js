...
const BudgetOptimizerNew = () => {
	...
	const budgetServiceUrl = process.env.REACT_APP_BUDGET_SERVICE_URL;
	const backendUrl = process.env.REACT_APP_BACKEND_URL;

	if (!budgetServiceUrl || !backendUrl) {
		throw new Error("Missing REACT_APP_BUDGET_SERVICE_URL or REACT_APP_BACKEND_URL in frontend .env");
	}
	...
	const fetchGeminiInsight = async (result, isRetry = false) => {
		...
		try {
			...
			const resp = await fetch(`${budgetServiceUrl}/api/budget/gemini-strategy`, {
				method: "POST",
	...
	const submitAnalysis = async () => {
		...
		try {
			...
			const response = await fetch(`${budgetServiceUrl}/api/budget/complete-analysis`, {
				method: "POST",
				headers: {
			...
			if (response.ok) {
				...
				try {
					...
					// Save to MongoDB via Node.js backend API
					const saveResponse = await fetch(`${backendUrl}/api/budget/save`, {
						method: "POST",
						headers: {
							"Content-Type": "application/json",
						},
	...
	const renderStepIndicator = () => {
		const steps = ["Account", "Academic", "Financial", "Food", "Transport", "Additional", "Results"];
			...
			<Card.Body className='p-4'>
				<Form>
					...
					<Form.Group className='mb-3'>
						...
						<Form.Control type='number' name='rent' value={formData.rent} onChange={handleInputChange} required />
		...
		<Card className='shadow-lg border-0'>
			...
			<Card.Body className='p-4'>
				<Form>
					...
					<Form.Group className='mb-3'>
						<Form.Label>
							<strong>Primary Transport Method *</strong>
						</Form.Label>
					...
					<Row>
						...
						<Col md={6}>
							<Form.Group className='mb-3'>
								<Form.Label>
									<strong>Home Visit Frequency</strong>
								</Form.Label>
								<Form.Select
					...
					<Form.Group className='mb-3'>
						<Form.Label>
							<strong>Transport Method for Home Visits</strong>
						</Form.Label>
						<Form.Select
							name='transport_method_home'
							value={formData.transport_method_home}
							onChange={handleInputChange}>
							<option value='Bus'>Bus</option>
							<option value='Train'>Train</option>
							<option value='Personal Vehicle'>Personal Vehicle</option>
					...
					<Row>
						<Col md={6}>
							<Form.Group className='mb-3'>
								...
								<Form.Control type='number' name='internet' value={formData.internet} onChange={handleInputChange} />
							</Form.Group>
						</Col>
						<Col md={6}>
							<Form.Group className='mb-3'>
					...
					<Row>
						...
						<Col md={6}>
							<Form.Group className='mb-3'>
								...
								<Form.Control type='number' name='utilities' value={formData.utilities} onChange={handleInputChange} />
							</Form.Group>
						</Col>
					</Row>
					...
					<Row>
						...
						<Col md={6}>
							<Form.Group className='mb-3'>
								...
								<Form.Control type='number' name='other' value={formData.other} onChange={handleInputChange} />
							</Form.Group>
						</Col>
					</Row>
				</Form>
							...
							<Col md={6}>
								...
								{calculated_budgets.transport && (
									<div className='mb-3 p-3 bg-light rounded'>
										...
										{/* Mode badge */}
										<div className='mb-2'>
											<span className='badge bg-secondary me-1'>{calculated_budgets.transport.transport_method}</span>
											...
											<div className='ps-2 border-start border-primary border-2'>
												{calculated_budgets.transport.breakdown.daily_commute > 0 && (
													<div className='d-flex justify-content-between'>
														...
														<small>
															<strong>
																LKR {calculated_budgets.transport.breakdown.daily_commute.toLocaleString()}
															</strong>
														</small>
													</div>
												)}
												{calculated_budgets.transport.breakdown.misc_trips > 0 && (
													<div className='d-flex justify-content-between'>
														<small className='text-muted'>
															...
															<span className='ms-1 text-muted' style={{ fontSize: "0.73rem" }}>
																(rain, late nights, city)
															</span>
														</small>
														<small>
															<strong>LKR {calculated_budgets.transport.breakdown.misc_trips.toLocaleString()}</strong>
														</small>
													</div>
												)}
												{calculated_budgets.transport.breakdown.home_visits > 0 && (
													<div className='d-flex justify-content-between'>
														...
														<small>
															<strong>LKR {calculated_budgets.transport.breakdown.home_visits.toLocaleString()}</strong>
														</small>
													</div>
												)}
											</div>
						...
						<Card.Body className='p-4'>
							...
							{analysisResult.optimal_strategy.maximum_savings_potential > 0 && (
								<Alert variant='success' className='mb-4'>
									<h5>
										💰 Total Savings Potential: LKR{" "}
										{analysisResult.optimal_strategy.maximum_savings_potential.toLocaleString()}/month
									</h5>
									...
									<div className='opt-journey-wrap'>
										...
										<div className='opt-journey-footer'>
											<p className='opt-journey-footer-text'>
												...
												<strong> OpenAI AI Enhanced Strategy</strong> section. There the AI will convert this path into
												a clear, step-by-step routine you can follow.
											</p>
													...
													<Card className='border-danger mb-3'>
														...
														<Card.Body>
															<ul className='mb-0 ps-3'>
																{analysisResult.optimal_strategy.implementation_plan.immediate_actions.map(
																	(action, idx) => (
																		<li key={idx} className='mb-2'>
																			<small>{action.action}</small>
																			<div className='text-success'>
																				<small>
																					<strong>Save: LKR {action.savings.toLocaleString()}</strong>
																				</small>
																			</div>
																		</li>
																	),
											...
											analysisResult.optimal_strategy.implementation_plan.this_month_actions.length > 0 && (
												<Col md={4}>
													<Card className='border-warning mb-3'>
														...
														<Card.Body>
															<ul className='mb-0 ps-3'>
																{analysisResult.optimal_strategy.implementation_plan.this_month_actions.map(
																	(action, idx) => (
																		<li key={idx} className='mb-2'>
																			<small>{action.action}</small>
																			<div className='text-success'>
											...
											analysisResult.optimal_strategy.implementation_plan.long_term_actions.length > 0 && (
												<Col md={4}>
													<Card className='border-info mb-3'>
														...
														<Card.Body>
															<ul className='mb-0 ps-3'>
																{analysisResult.optimal_strategy.implementation_plan.long_term_actions.map(
																	(action, idx) => (
																		<li key={idx} className='mb-2'>
																			<small>{action.action}</small>
																			<div className='text-success'>
										...
										<div className='gemini-footer-note'>
											...
											{!isStreaming && (
												<Button
													size='sm'
													variant='link'
													className='ms-3 p-0 gemini-retry-btn'
													onClick={() => fetchGeminiInsight(analysisResult, true)}>
													🔄 Regenerate
												</Button>
											)}
