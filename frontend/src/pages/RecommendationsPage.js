									...
									<ListGroup>
										{recommendations.map((place, index) => (
											<ListGroup.Item key={index}>
												<strong>{place.name}</strong> - Rating: {place.rating || "N/A"}, Temperature:{" "}
												{place.temperature || "N/A"}°C
												{place.formatted_address && <div className='text-muted small'>{place.formatted_address}</div>}
											</ListGroup.Item>
										))}
									</ListGroup>
									<Button variant='success' size='lg' className='mt-4' onClick={handleShowBestRecommendation}>
