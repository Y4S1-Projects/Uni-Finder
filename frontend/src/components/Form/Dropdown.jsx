...
const Dropdown = ({
  ...
  }, [JSON.stringify(defaultOptions), multiple]);
  ...
  const handleRemoveOption = (optionToRemove, e) => {
    ...
    const newSelectedOptions = selectedOptions.filter(
      (item) => item[idKey] !== optionToRemove[idKey]
    );
  ...
  const handleBlur = (e) => {
    const isRemoveButtonClick = e.relatedTarget?.closest(
  ...
  const getDisplayValues = (options) => {
    if (!options || options.length === 0) return "";
    return options.map((option) => getDisplayValue(option)).join(", ");
  };
        ...
        {selectedOptions.map((option) => (
          ...
          >
            ...
            <span
              ...
              className="flex-shrink-0 w-3 h-3 sm:w-4 sm:h-4 flex items-center justify-center rounded-full transition-colors cursor-pointer hover:bg-gray-600"
              role="button"
              tabIndex={0}
              onKeyDown={(e) => {
                if (e.key === "Enter" || e.key === " ") {
    ...
    <div className={twMerge("flex flex-col w-full", outerContainerStyle)}>
      ...
      <div ref={dropdownRef} className={`relative ${wrapperClassName}`}>
        ...
        >
          {multiple ? (
            ...
            >
              ...
              {isSearchable && isSearchMode && isOpen && (
        ...
        {isOpen && (
          <div
            className={twMerge(
              "absolute mt-1 w-full z-10 bg-white rounded-md shadow-lg",
              dropdownClassName
            )}
            role="listbox"
          >
                ...
                <div
                  ...
                  className={twMerge(
                    ...
                    isOptionSelected(option)
                      ...
                      : "",
                    focusedIndex === index && !isOptionSelected(option)
                      ? "bg-purple-50"
                      : ""
                  )}
                  title={getDisplayValue(option)}
