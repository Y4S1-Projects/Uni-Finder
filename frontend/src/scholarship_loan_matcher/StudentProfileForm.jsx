...
// A/L Streams
const AL_STREAMS = ["Physical Science", "Bio Science", "Commerce", "Arts", "Technology", "Other"];

// English Result Options
const ENGLISH_RESULTS = ["A", "B", "C", "S", "F"];

// University Registration Status
const UNI_REGISTRATION_STATUS = ["Not Registered", "State University", "Non-State / Private"];
...
const INSTITUTE_TYPES = ["NSHEI (Degree Awarding Institute)", "Private University", "Foreign University"];

// Guarantor Options
const GUARANTOR_OPTIONS = ["None", "1 Guarantor", "2 Guarantors"];

// Employment Status
const EMPLOYMENT_STATUS = ["Unemployed", "Employed Full-time"];
...
export default function StudentProfileForm({
	...
	const handleSubmit = (event) => {
		...
		if (isScholarship) {
			...
			formatted = {
				...
				field_of_study:
					scholarshipValues.preferredFieldOfStudy === "Other" ? scholarshipValues.preferredFieldOther
				...
				al_stream:
					scholarshipValues.alStream === "Other" ? scholarshipValues.alStreamOther : scholarshipValues.alStream,
			...
			formatted = {
				...
				field_of_study:
					loanValues.preferredFieldOfStudy === "Other" ? loanValues.preferredFieldOther
	...
	const renderError = (field) =>
		touched[field] && errors[field] ? <span className='matcher-form__error'>{errors[field]}</span> : null;
			...
			<form className='matcher-form' onSubmit={handleSubmit} noValidate>
				<div className='matcher-form__section'>
					...
					<div className='matcher-form__grid'>
						<label>
							A/L Stream*
							<select name='alStream' value={scholarshipValues.alStream} onChange={handleChange} onBlur={handleBlur}>
						...
						<label>
							District*
							<select name='district' value={scholarshipValues.district} onChange={handleChange} onBlur={handleBlur}>
		...
		<form className='matcher-form' onSubmit={handleSubmit} noValidate>
			<div className='matcher-form__section'>
				...
				<div className='matcher-form__grid'>
					<label>
						G.C.E. A/L Year*
						<select name='alYear' value={loanValues.alYear} onChange={handleChange} onBlur={handleBlur}>
