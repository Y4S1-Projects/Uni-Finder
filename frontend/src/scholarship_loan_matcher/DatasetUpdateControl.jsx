...
export default function DatasetUpdateControl() {
	...
	const getScholarshipCount = () => {
		...
		return (
			summary.cleaner_results?.scholarships?.rows_output || summary.scraper_results?.scholarships_collected || null
		);
	};

	const getLoanCount = () => {
		if (!summary) return null;
		return summary.cleaner_results?.loans?.rows_output || summary.scraper_results?.loans_collected || null;
	};

	const getNewScholarshipCount = () => {
		if (!summary) return null;
		return (
			summary.cleaner_results?.scholarships?.new_unique_count ??
	...
	const getNewLoanCount = () => {
		if (!summary) return null;
		return summary.cleaner_results?.loans?.new_unique_count ?? summary.cleaner_results?.loans?.new_count ?? null;
	};
	...
	return (
		<div className='dataset-update-control'>
			...
			<div className='dataset-update-control__actions'>
				<button className='dataset-update-control__btn' onClick={() => setShowConfirm(true)} disabled={isLoading}>
					{isLoading ?
						<>
				...
				<div className='dataset-update-control__modal-backdrop'>
					<div className='dataset-update-control__modal'>
						...
						<p className='dataset-update-control__modal-body'>
							You are about to refresh the live scholarship and loan datasets. This process runs a full scraping and
							cleaning pipeline and can take approximately 10–20 minutes to complete. During this time, the server will
							be busy but students can still browse using the existing data.
						</p>
						<p className='dataset-update-control__modal-body'>Are you sure you want to start the update now?</p>
			...
			{updateStatus === "success" && summary && (
				<div className='dataset-update-control__success'>
					<p className='dataset-update-control__success-message'>✓ Dataset update completed successfully!</p>
					<p className='dataset-update-control__current-stats-title'>
					...
					<div className='dataset-update-control__stats'>
						...
						{summary.duration_seconds && (
							<div className='dataset-update-control__stat-item'>
								...
								<span className='dataset-update-control__stat-value'>{Math.round(summary.duration_seconds)}s</span>
							</div>
						)}
					</div>
				</div>
			...
			{updateStatus === "error" && (
				<div className='dataset-update-control__error'>
					<p className='dataset-update-control__error-message'>✗ Update failed: {errorMessage}</p>
				</div>
			)}
		</div>
	);
